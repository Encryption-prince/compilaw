...
const { scanForConsent } = require("./consent-scanner");
const { scanConfigPatterns } = require("./config-scanner");
        ...
        if (stats.isDirectory()) {
            ...
            try {
                ...
                if (ext === ".py") {
                    ...
                    // Config/infrastructure checks on every JS/TS file
                    const configFindings = scanConfigPatterns(fullPath, content);
...
async function main() {
    ...
    // ── Consent surface scan ──────────────────────────────────────────────
    const piiFindings = filteredFindings.filter(f => f.type === "declaration");
    const { consentFindings, consentSignals, hasConsent } = scanForConsent(targetFolder, piiFindings);
    findings.push(...consentFindings);

    if (hasConsent) {
        console.log(chalk.green(`  ✓ Consent surface detected (${consentSignals.join(", ")})`));
    } else if (piiFindings.length > 0) {
        console.log(chalk.red(`  ✗ No consent mechanism found — ${piiFindings.length} PII field(s) detected`));
    }

    const allFindings = findings.filter((f) => !config.ignoreCategories.includes(f.category));

    const dependencyResults = scanDependencies(targetFolder, installDeps);

    function buildReport(findings, context, dependencyResults) {        const grouped = {};
        ...
        for (const category in grouped) {
            ...
            const rule = DPDP_RULES[category] || (items[0] && items[0].rule) || null;
            ...
            for (const item of items) {
                ...
                let flowTag = "";
                if (item.type === "data-flow") {
                    if (item.subtype === "logging-risk")         flowTag = " [DATA FLOW — PII in logs]";
                    else if (item.subtype === "response-exposure") flowTag = " [DATA FLOW — PII in HTTP response]";
                    else if (item.externalRisk)                  flowTag = " [DATA FLOW — possible external transmission]";
                    else                                         flowTag = " [DATA FLOW — internal call]";
                } else if (item.type === "config-risk") {
                    flowTag = ` [CONFIG — ${item.subtype || "infrastructure risk"}]`;
                } else if (item.type === "consent-gap") {
                    flowTag = " [CONSENT GAP]";
                }
    ...
    const reportText = buildReport(allFindings, context, dependencyResults);

    const severityCounts = { Critical: 0, High: 0, Medium: 0, Low: 0 };
    for (const finding of allFindings) {
        const rule = finding.rule || DPDP_RULES[finding.category];
        if (rule && severityCounts[rule.severity] !== undefined) {
    ...
    const reportData = {
        ...
        findings: allFindings.map((f) => ({ ...f, rule: f.rule || DPDP_RULES[f.category] || null })),
    ...
    if (dashboardUrl) {
        fetch(dashboardUrl, {
            ...
            .then((data) => {
                console.log(chalk.bold(`\nScan complete — ${allFindings.length} findings (Critical: ${severityCounts.Critical}, High: ${severityCounts.High}, Medium: ${severityCounts.Medium}, Low: ${severityCounts.Low})`));
